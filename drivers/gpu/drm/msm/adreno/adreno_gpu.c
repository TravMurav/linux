		...
		struct msm_gpu *gpu)
{
	...
	if (ret == -ENODEV) {
		...
		if (adreno_is_a2xx(adreno_gpu)) {
			dev_warn(dev, "Unable to set the OPP table. Falling back to 200 MHz.\n");
			dev_pm_opp_add(dev, 200000000, 0);
		} else if (adreno_is_a320(adreno_gpu)) {
			dev_warn(dev, "Unable to set the OPP table. Falling back to 320 MHz.\n");
			dev_pm_opp_add(dev, 320000000, 0);
		} else {
