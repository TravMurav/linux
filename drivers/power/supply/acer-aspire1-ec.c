...
static const char* aspire_ec_psy_battery_model[] = {
	"AP18C4K",
	"AP18C8K",
	"AP19B8K",
	"AP16M4J",
...
static const char* aspire_ec_psy_battery_vendor[] = {
	"SANYO",
	"SONY",
	"PANASONIC",
	"SAMSUNG",
...
static int aspire_ec_ram_read(struct i2c_client *client, u8 off, u8 *data, u8 data_len)
{
	...
	i2c_smbus_write_i2c_block_data(client, ASPIRE_EC_RAM_READ, sizeof(tmp), tmp);
...
static irqreturn_t aspire_ec_irq_handler(int irq, void *data)
{
	 ...
	 * transfer is not possible yet. So this delay allows us to supress
...
void aspire_ec_bridge_hpd_enable(struct drm_bridge *bridge)
{
...
static SIMPLE_DEV_PM_OPS(aspire_ec_pm_ops, NULL, aspire_ec_resume);
